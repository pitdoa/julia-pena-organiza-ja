...
const JujuModule = ({ onBack }: JujuModuleProps) => {
  ...
  const handleClearHistory = async (dateToClear: string) => {
    ...
    const start = new Date(`${dateToClear}T00:00:00.000Z`);
    const end = new Date(`${dateToClear}T23:59:59.999Z`);
    ...
    if (error) {
      toast({ title: "Erro ao apagar histórico", description: error.message, variant: "destructive" });
      return;
    }
            ...
            <div className="space-y-1 pr-2">
              {Array.from(conversations.keys())
                ...
                .map(dateKey => (
                  <Button
                    key={dateKey}
                    variant={activeDate === dateKey ? "secondary" : "ghost"}
        ...
        <div className="col-span-9 rounded-lg border flex flex-col h-full min-h-0">
          <div className="p-2 border-b flex justify-between items-center flex-shrink-0">
            <h3 className="font-semibold text-sm px-2">
              {`Relembrando: ${format(parseISO(activeDate), "PPP", { locale: ptBR })}`}
            </h3>
            {messagesToShow.length > 0 && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Limpar histórico deste dia?</AlertDialogTitle>
                    <AlertDialogDescription>Esta ação não pode ser desfeita.</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancelar</AlertDialogCancel>
                    <AlertDialogAction
                      onClick={() => handleClearHistory(activeDate)}
